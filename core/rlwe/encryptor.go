...
// ============================ 【中文文件说明】 ============================
// 本文件实现 RLWE 加密器（[CKKS2017] Algorithm 2 - Encrypt）。
//
// 加密算法（使用公钥）:
//   1. 采样随机多项式 u ← χ_secret（小范数分布）
//   2. 采样噪声多项式 e0, e1 ← χ_error（离散高斯分布）
//   3. 计算:
//      c0 = u·pk₀ + e0 + m (mod Q)
//      c1 = u·pk₁ + e1 (mod Q)
//   4. 返回 ct = (c0, c1)
...
//
// 加密的本质：ct = Enc(0) + (m, 0)
// 即先加密一个零密文（噪声掩盖），再把明文加到第一个分量上。
// 安全性基于 RLWE（Ring Learning With Errors）问题的困难性。
// =========================================================================
