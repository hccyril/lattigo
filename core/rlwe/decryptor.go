...
// ============================ 【中文文件说明】 ============================
// 本文件实现 RLWE 解密器（[CKKS2017] Algorithm 3 - Decrypt）。
//
// 解密算法:
//   输入: ct = (c0, c1), 私钥 s
//   计算: m' = c0 + c1·s (mod Q)
...
//
// 解密原理推导:
//   加密时: c0 = m + e0 + e1·pk₀, c1 = e1·pk₁
//   而公钥: pk₀ = -a·s + e_pk, pk₁ = a（由私钥派生）
//   所以: c0 + c1·s = m + e0 + e1·(-a·s + e_pk) + e1·a·s
//                      = m + e0 + e1·e_pk
//                      ≈ m + 小噪声
//   解密结果 ≈ m·Δ + 噪声，噪声足够小以保证 ⌊m'/Δ⌉ = m
//
// 对于度 > 1 的密文（乘法后）:
//   m' = c0 + c1·s + c2·s² + ... + cd·s^d (mod Q)
// =========================================================================
